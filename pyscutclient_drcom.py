...
p_md5 = Ether(src=MY_MAC, dst=DST_MAC, type=0x888e)/EAPOL(version=1, type=0, len=43)/ \
        EAP(code=2, type=4, id=0, len=43)/Raw(load='\x10' + 'reseverd_for_md5' + username + '\x00Da*\x00' + MY_IP_HEX)/ \
        Padding(load=35 * '\x00')
...
def update_md5_packet(server_md5_info):
    ...
    p_md5 = Ether(src=MY_MAC, dst=DST_MAC, type=0x888e)/EAPOL(version=1, type=0, len=43)/ \
            ...
            Raw(load='\x10' + md5('\x00' + password + server_md5_info).digest() + username + '\x00Da*\x00' + MY_IP_HEX)/ \
            Padding(load=35 * '\x00')
...
def sniff_handler(pkt):
    ...
    try:
        ...
        elif pkt.haslayer(UDP) and pkt.dport == 61440 and pkt.load[4] == DRCOM_RESPONSE_INFO:
